...
/**********************************************************/
// Name: TimestampsUserid
// Description: Initialize the http_listner object with uri and bind the methods with client call
/**********************************************************/
TimestampsUserid::TimestampsUserid(utility::string_t url) : m_listener(url)
{
	m_listener.support(methods::GET, std::bind(&TimestampsUserid::handle_get, this, std::placeholders::_1));
...
/**********************************************************/
void TimestampsUserid::handle_get(http_request request)
{
	TRACE(L"\nhandle GET\n");
	...
	pqxx::result R = pghelper.get_userid_from_timestamps(fltID, type, seatnum);

	// Send response as json object
	auto answer = json::value::object();

	answer[L"userid"] = json::value::string(to_utf16string(R[0][0].c_str()));
